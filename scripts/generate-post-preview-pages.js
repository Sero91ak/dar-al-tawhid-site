...
function buildHtml(markdown, postId) {
  ...
  const appUrl = `${SITE}/#/beitraege/${encodeURIComponent(postId)}`;
  return `<!doctype html>
<html lang="de">
<head>
  <meta charset="utf-8">
...
<body><main class="page"><img class="logo" src="${SITE}/logo.png" alt="DAR AL TAWḤID"><h1>${htmlAttr(title)}</h1><p>Der Beitrag wird geöffnet. Falls die Weiterleitung nicht automatisch funktioniert, bitte den Button benutzen.</p><a class="button" href="${htmlAttr(appUrl)}">Beitrag öffnen</a><div class="site">dar-al-tawhid.de</div></main></body>
</html>
`;
}
