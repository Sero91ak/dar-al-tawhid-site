#!/usr/bin/env node
/* DAR AL TAWḤID – OneSignal broadcast: Zakāt-Rechner 72h Fokus (live). */
...
function buildZakatPushUrl() {
  const site = siteOriginFromEnv(SITE_URL);
  const v = Date.now();
  return `${site}/?focus=zakat&v=${encodeURIComponent(v)}#zakat`;
}
...
async function sendWithFallbacks(basePayload) {
  ...
  for (const payload of attempts) {
    try {
      ...
      console.log(`Zakāt-Fokus-Push gesendet (${target}):`, result.text);
      return result;
    } catch (err) {
      lastError = err;
      console.warn("Zakāt-Push Versuch fehlgeschlagen:", err.message || err);
    }
  }
  throw lastError || new Error("Zakāt-Fokus-Push fehlgeschlagen");
}

(async function main() {
  const url = buildZakatPushUrl();
  const title = "🕌 Zakāt-Rechner jetzt online";
  const message = "72 Std. im Fokus · Live-Goldpreise · Nisāb · kompakt · Zahlentastatur. Tippe hier zum Öffnen.";
