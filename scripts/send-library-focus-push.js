#!/usr/bin/env node
/* DAR AL TAWḤID – OneSignal broadcast: Bibliothek 72h Fokus (live). */
...
function buildLibraryPushUrl() {
  const site = siteOriginFromEnv(SITE_URL);
  const v = Date.now();
  return `${site}/?focus=library&v=${encodeURIComponent(v)}#bibliothek`;
}
...
async function sendWithFallbacks(basePayload) {
  ...
  for (const payload of attempts) {
    try {
      ...
      console.log(`Bibliothek-Fokus-Push gesendet (${target}):`, result.text);
      return result;
    } catch (err) {
      lastError = err;
      console.warn("Bibliothek-Push Versuch fehlgeschlagen:", err.message || err);
    }
  }
  throw lastError || new Error("Bibliothek-Fokus-Push fehlgeschlagen");
}

(async function main() {
  const url = buildLibraryPushUrl();
  const title = "📚 DAR AL TAWḤĪD Bibliothek jetzt online";
  const message = "72 Std. im Fokus · PDFs & Lernmaterialien · direkt lesen, speichern und offline nutzen. Tippe hier zum Öffnen.";
  ...
  const payload = withNotificationIcons({
    ...
    data: {
      ...
      url,
      focus: "library-72h",
      newsId: "dar-al-tawhid-library-live-2026",
      badge: "72 Std. im Fokus"
    },
    name: `library-focus-live-${RUN_ID}`
  }, SITE_URL);
