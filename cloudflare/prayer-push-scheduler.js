...
function enforcePrayerCopyGuard(copy) {
  ...
  const blocked = hasBlockedPrayerPhrase(title) || hasBlockedPrayerPhrase(body);
  return {
    headings: {
      de: title,
...
export async function runPrayerPushScheduler(env, options = {}, deps = {}) {
  ...
  const dedupedRows = dedupeRegistrations(rows);
  const groups = groupRegistrations(dedupedRows, onlySub);
  const userCount = dedupedRows.filter((r) => !onlySub || String(r.subscription_id) === onlySub).length;
  const duplicateRegistrationCount = Math.max(0, rawRows.length - dedupedRows.length);
  stats.duplicates += duplicateRegistrationCount;
  ...
  const statusReport = {
    ...
    userRegistry: dedupedRows.slice(0, 50).map((r) => ({
      subscriptionId: r.subscription_id,
      timezone: r.timezone,
      city: r.city,
      lastSyncedAt: r.last_synced_at
    })),
    rawRegistrations: rawRows.length,
    dedupedRegistrations: dedupedRows.length
  };
