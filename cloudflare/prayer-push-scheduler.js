...
const SCHEDULE_LOOKAHEAD_MINUTES = 48 * 60;
...
function prayerNotificationCopy(prayer, mode, group, sendAfter) {
  ...
  return {
    headings: { de: prayerNotificationTitle(prayer, mode, group), en: prayerNotificationTitle(prayer, mode, group) },
...
export async function runPrayerPushScheduler(env, options = {}, deps = {}) {
  ...
  const statusReport = {
    ...
    schedulerEngine: "cloudflare-worker",
  ...
  let statusWrite = { saved: false, reason: "Kein GitHub-Write" };
  if (deps.githubGet && deps.githubPut) {
    statusWrite = await writePrayerPushStatus(env, statusReport, deps);
  }
