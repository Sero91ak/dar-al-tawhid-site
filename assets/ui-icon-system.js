/* DAR AL TAWḤĪD – Premium UI Icon System (Navigation & Controls) (SVG, theme-aware via currentColor) */
(function (global) {
  ...
  function appIconEmblem(name, size) {
    size = size || 'lg';
    return '<span class="ui-icon app-icon-emblem emoji-emblem" aria-hidden="true">' + appIcon(name, size) + '</span>';
  }
  ...
  function hydrateAppIcons(root) {
    ...
    scope.querySelectorAll('[data-app-icon]').forEach(function (el) {
      ...
      if (!name) return;
      el.innerHTML = uiIcon(name, size, el.className.replace('nav-icon', '').trim());
      el.removeAttribute('data-app-icon');
      el.removeAttribute('data-app-icon-size');
    });
  }

  global.uiIcon = appIcon;
  ...
  global.hydrateUiIcons = hydrateAppIcons;
