...
RAW="https://raw.githubusercontent.com/Sero91ak/dar-al-tawhid-site/05179edf5f1733bae0e26a0a1629294f89cd4c55"
...
curl -fsSL "$RAW/voice-studio/local-engine.py?v=212" -o "$STAGE/local-engine.py"
curl -fsSL "$RAW/voice-studio/index.html?v=212" -o "$STAGE/studio.html"
curl -fsSL "$RAW/data/pronunciation/pronunciation-rules.json?v=212" -o "$STAGE/pronunciation-rules.json"
curl -fsSL "$RAW/data/pronunciation/voice-production-profile.json?v=212" -o "$STAGE/voice-production-profile.json"
curl -fsSL "$RAW/data/pronunciation/voice-regression-fixtures.json?v=212" -o "$STAGE/voice-regression-fixtures.json"
curl -fsSL "$RAW/scripts/voice-studio/validate-v2.py?v=212" -o "$STAGE/validate-v2.py"
curl -fsSL "$RAW/watermark-my-logo-full.png?v=212" -o "$STAGE/watermark-my-logo-full.png" || true
curl -fsSL "$RAW/app-icon-512.png?v=212" -o "$STAGE/app-icon-512.png" || true
...
if ! "$PY" "$STAGE/validate-v2.py"     "$STAGE/pronunciation-rules.json"     "$STAGE/voice-production-profile.json"     "$STAGE/local-engine.py"     "$STAGE/voice-regression-fixtures.json"; then
  echo "FEHLER: Voice-Studio-2.1.2-Regressionsprüfung fehlgeschlagen. Alte Installation bleibt unverändert."
  exit 1
fi
...
echo "Voice Studio 2.1.2 Validierung bestanden. Backup: $BACKUP"
...
final class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        config.applicationNameForUserAgent = "DĀRVoiceStudioMac/2.1.2"
...
say_status "DĀR Voice Studio 2.1.2 ist installiert."
open -n "$APP"
