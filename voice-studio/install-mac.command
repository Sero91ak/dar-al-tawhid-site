...
RAW="https://raw.githubusercontent.com/Sero91ak/dar-al-tawhid-site/daf5f8a76724c58e34dc1e3421d1c9e086882be7"
...
curl -fsSL "$RAW/voice-studio/local-engine.py?v=273" -o "$STAGE/local-engine.py"
curl -fsSL "$RAW/voice-studio/index.html?v=273" -o "$STAGE/studio.html"
curl -fsSL "$RAW/voice-studio/content-studio.js?v=273" -o "$STAGE/content-studio.js"
curl -fsSL "$RAW/data/pronunciation/pronunciation-rules.json?v=273" -o "$STAGE/pronunciation-rules.json"
curl -fsSL "$RAW/data/pronunciation/voice-production-profile.json?v=273" -o "$STAGE/voice-production-profile.json"
curl -fsSL "$RAW/data/pronunciation/islamic-master-library.json?v=273" -o "$STAGE/islamic-master-library.json"
curl -fsSL "$RAW/data/pronunciation/voice-regression-fixtures.json?v=273" -o "$STAGE/voice-regression-fixtures.json"
curl -fsSL "$RAW/scripts/voice-studio/validate-v2.py?v=273" -o "$STAGE/validate-v2.py"
curl -fsSL "$RAW/watermark-my-logo-full.png?v=273" -o "$STAGE/watermark-my-logo-full.png" || true
curl -fsSL "$RAW/app-icon-512.png?v=273" -o "$STAGE/app-icon-512.png" || true
...
if ! "$PY" "$STAGE/validate-v2.py"     "$STAGE/pronunciation-rules.json"     "$STAGE/voice-production-profile.json"     "$STAGE/local-engine.py"     "$STAGE/voice-regression-fixtures.json"; then
  echo "FEHLER: Voice-Studio-2.7.3-Regressionsprüfung fehlgeschlagen. Alte Installation bleibt unverändert."
  exit 1
fi
...
echo "Voice Studio 2.7.3 Validierung bestanden. Backup: $BACKUP"
...
final class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        config.applicationNameForUserAgent = "DĀRVoiceStudioMac/2.7.3"
...
<dict>
  ...
  <key>CFBundleShortVersionString</key><string>2.7.3</string>
...
say_status "DĀR Voice Studio 2.7.3 ist installiert."
if ! open -n "$APP"; then
