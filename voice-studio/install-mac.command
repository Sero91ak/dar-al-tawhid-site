...
RAW="https://raw.githubusercontent.com/Sero91ak/dar-al-tawhid-site/5c571278958578c4052c3e6e9fd3627cf76db12a"
...
curl -fsSL "$RAW/voice-studio/local-engine.py?v=211" -o "$STAGE/local-engine.py"
curl -fsSL "$RAW/voice-studio/index.html?v=211" -o "$STAGE/studio.html"
curl -fsSL "$RAW/data/pronunciation/pronunciation-rules.json?v=211" -o "$STAGE/pronunciation-rules.json"
curl -fsSL "$RAW/data/pronunciation/voice-production-profile.json?v=211" -o "$STAGE/voice-production-profile.json"
curl -fsSL "$RAW/data/pronunciation/voice-regression-fixtures.json?v=211" -o "$STAGE/voice-regression-fixtures.json"
curl -fsSL "$RAW/scripts/voice-studio/validate-v2.py?v=211" -o "$STAGE/validate-v2.py"
curl -fsSL "$RAW/watermark-my-logo-full.png?v=211" -o "$STAGE/watermark-my-logo-full.png" || true
curl -fsSL "$RAW/app-icon-512.png?v=211" -o "$STAGE/app-icon-512.png" || true
...
if ! "$PY" "$STAGE/validate-v2.py"     "$STAGE/pronunciation-rules.json"     "$STAGE/voice-production-profile.json"     "$STAGE/local-engine.py"     "$STAGE/voice-regression-fixtures.json"; then
  echo "FEHLER: Voice-Studio-2.1.1-Regressionsprüfung fehlgeschlagen. Alte Installation bleibt unverändert."
  exit 1
fi
...
echo "Voice Studio 2.1.1 Validierung bestanden. Backup: $BACKUP"
...
final class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        config.applicationNameForUserAgent = "DĀRVoiceStudioMac/2.1.1"
...
say_status "DĀR Voice Studio 2.1.1 ist installiert."
open -n "$APP"
