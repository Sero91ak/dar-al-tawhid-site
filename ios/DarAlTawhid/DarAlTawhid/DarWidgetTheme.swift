...
enum DarWidgetTheme {
    ...
    static func palette(for snap: DarWidgetSnapshot, look: DarWidgetLook = .app) -> DarWidgetPalette {
        switch look {
        case .app:
            if let custom = customFromSnapshot(snap) { return custom }
    ...
    private static func customFromSnapshot(_ snap: DarWidgetSnapshot) -> DarWidgetPalette? {
        ...
        let muted = Color(hex: snap.mutedHex) ?? cream.opacity(0.62)
        let panel = ink.opacity(0.92)
        let luminance = 0.2126 * ink.components.red + 0.7152 * ink.components.green + 0.0722 * ink.components.blue
        return DarWidgetPalette(
            ...
            cream: cream,
            muted: muted,
            line: gold.opacity(0.28),
            darkText: luminance > 0.55
        )
    }
...
extension Color {
    ...
    fileprivate var components: (red: Double, green: Double, blue: Double) {
        var r: CGFloat = 0
        var g: CGFloat = 0
        var b: CGFloat = 0
        var a: CGFloat = 0
